...
for(t in 9:1335){
  ...
  if(child_data[T,t] == 0){
    #周りも発言していない
    if(label_sub == 0){
      choice0 <- append(choice0, child_data[T,t+1])
      #発話すれば報酬を、発話しなければ報酬なし
      if(child_data[T,t+1] == 1){
    ...
    #周りは関係ある話をしている
      ...
      choice1 <- append(choice1, child_data[T,t+1])
      if(child_data[T,t+1] == 1){
    ...
    #周りは関係ない話をしている
      ...
      choice2 <- append(choice2, child_data[T,t+1])
      if(child_data[T,t+1] == 1){
    ...
    #周りは関係ある話とない話をしている
      ...
      choice3 <- append(choice3, child_data[T,t+1])
      if(child_data[T,t+1] == 1){
  ...
  #ターゲット(A=2)は発話している
  else if(child_data[T,t] == 1){
    #ターゲットA(視線17)は関係ある話をしている
    if((child_data[T+15,t] == 3) || (child_data[T+15,t] == 4) || (child_data[T+15,t] == 7)){
      #周りは発言していない
      if(label_sub == 0){
        choice4 <- append(choice4, child_data[T,t+1])
        if(child_data[T,t+1] == 1){
      ...
      #周りは関係ある話をしている
        ...
        choice5 <- append(choice5, child_data[T,t+1])
        if(child_data[T,t+1] == 1){
      ...
      #周りは関係ない話をしている
        ...
        choice6 <- append(choice6, child_data[T,t+1])
        if(child_data[T,t+1] == 1){
      ...
      #周りは関係ある話とない話をしている
        ...
        choice7 <- append(choice7, child_data[T,t+1])
        if(child_data[T,t+1] == 1){
    ...
    #ターゲットA(視線17)は関係ない話をしている
    else if((child_data[T+15,t] == 5) || (child_data[T+15,t] == 6) || (child_data[T+15,t] == 8)){
      #周りは発言していない
      if(label_sub == 0){
        choice8 <- append(choice8, child_data[T,t+1])
        if(child_data[T,t+1] == 1){
      ...
      #周りは関係ある話をしている
        ...
        choice9 <- append(choice9, child_data[T,t+1])
        if(child_data[T,t+1] == 1){
      ...
      #周りは関係ない話をしている
        ...
        choice10 <- append(choice10, child_data[T,t+1])
        if(child_data[T,t+1] == 1){
      ...
      #周りは関係ある話とない話をしている
        ...
        choice11 <- append(choice11, child_data[T,t+1])
        if(child_data[T,t+1] == 1){
...
func_qlearning <- function(param, data, prior = NULL)
{
  ...
  for(t in 1:T){
    ...
    #ソフトマックスで選択肢（発言しない）の選択確率を決定する(Q[1]発言しない、Q[2]発言する)
...
func_minimize <- function(modelfunc, param, data, prior)
{
  ...
  for(i in 1:12){
    if(length(data[[1]][[i]]) > 0){
      Data <- list(reward=data[[1]][[i]], choice=data[[2]][[i]])
...
# 最尤推定 --------------------------------------------------------------------
#行動と報酬のリスト作成
choice <- list(as.numeric(choice0),as.numeric(choice1),as.numeric(choice2),as.numeric(choice3),as.numeric(choice4),as.numeric(choice5),as.numeric(choice6),as.numeric(choice7),as.numeric(choice8),as.numeric(choice9),as.numeric(choice10),as.numeric(choice11))
reward <- list(reward0,reward1,reward2,reward3,reward4,reward5,reward6,reward7,reward8,reward9,reward10,reward11)
data <- list(reward, choice)
